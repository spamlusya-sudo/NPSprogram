...
function sanitizeState(input) {
  ...
  return {
    ...
    stockChartMode: ["history", "future"].includes(state.stockChartMode) ? state.stockChartMode : "history",
    forecastChartMode: ["history", "future"].includes(state.forecastChartMode) ? state.forecastChartMode : "future",
...
export default async (req) => {
  ...
  if (req.method === "POST") {
    ...
    const expectedUpdatedAt = payload?.lastLoadedAt || null;
    const current = await store.get(key, { type: "json" });
    const currentUpdatedAt = current?.updatedAt || null;

    if (current && expectedUpdatedAt && currentUpdatedAt && expectedUpdatedAt !== currentUpdatedAt) {
      return json(
        {
          error: "stale_write",
          serverState: sanitizeState(current.state),
    ...
    await store.setJSON(key, { state: incomingState, updatedAt });
